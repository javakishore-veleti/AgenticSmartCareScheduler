...
## Phase 3: Paper Evidence Generation — **MOSTLY DONE**

### Activity Timeline (Mar 25-31)

| # | Activity | Time | Status |
|---|---|---|---|
| 1 | PCA→COA agent pipeline on Kaggle data (C_p + channel) | 1.5 hr | **DONE** — runs via Airflow DAG, results in DB |
| 2 | Channel distribution chart | 0.5 hr | **DONE** — generated by DAG, shown in detail page |
| 3 | Baseline comparison (multi-channel vs SMS-only) | 1 hr | **DONE** — 54.6% vs 77.1% (+22.5pp) |
| 4 | Take portal screenshots (Fig. 2, Fig. 3) | 0.5 hr | PENDING |
| 5 | Paper V06 → V07 → V08 with real metrics | 2 hr | **DONE** — V08 committed with .docx |
...
| # | Criterion | Status |
|---|---|---|
| 1 | Real F1 and AUC from PCA risk model | **DONE** (F1=0.454, AUC=0.611) |
| 2 | Real C_p distribution from agent pipeline | **DONE** (48.4% mobile, 49.8% stationary, 1.8% unreachable) |
| 3 | Real channel selection stats | **DONE** (IVR 48.4%, SMS 49.8%, Callback 1.8%) |
| 4 | Channel distribution chart | **DONE** (generated by Airflow DAG, shown in admin UI) |
| 5 | Portal screenshots (Fig. 2, Fig. 3) | PENDING |
| 6 | GitHub Actions CI/CD + AWS IaC | **DONE** (23 AWS GH Actions + 10 CloudFormation templates) |
| 7 | Paper V08 with real numbers, guard logic, human-in-loop | **DONE** (.md + .docx) |
...
### Phase A: GitHub Actions — Infrastructure — **ALL DONE**

All 14 GH Action YAML files created. All manual-trigger only (workflow_dispatch). No auto-run on push.

| # | Task | GH Action | CloudFormation | Status |
|---|---|---|---|---|
| A1 | Validate VPC | `AWS-001-Validate-VPC.yml` | N/A | **DONE** |
| A2 | Create S3 bucket | `AWS-002-Create-S3-Bucket.yml` | `infra/cfn-s3-bucket.yaml` | **DONE** |
| A3 | Create SNS topics | `AWS-003-Create-SNS-Topics.yml` | `infra/cfn-sns-topics.yaml` | **DONE** |
| A4 | Create SQS queues | `AWS-004-Create-SQS-Queues.yml` | `infra/cfn-sqs-queues.yaml` | **DONE** |
| A5 | Create EventBridge | `AWS-005-Create-EventBridge.yml` | `infra/cfn-eventbridge.yaml` | **DONE** |
| A6 | Create OpenSearch | `AWS-006-Create-OpenSearch.yml` | `infra/cfn-opensearch.yaml` | **DONE** |
| A7 | Create Bedrock Agent | `AWS-007-Create-Bedrock-Agent.yml` | `infra/cfn-bedrock-agent.yaml` | **DONE** |
| A8 | Create Lambda Agents | `AWS-008-Create-Lambda-Agents.yml` | `infra/cfn-lambda-agents.yaml` | **DONE** |
| A9 | Create Step Functions | `AWS-009-Create-StepFunctions.yml` | `infra/cfn-step-functions.yaml` | **DONE** |
| A10 | Create Connect | `AWS-010-Create-Connect.yml` | `infra/cfn-connect.yaml` | **DONE** |
| A11 | Create HealthLake | `AWS-010-Create-HealthLake.yml` | `infra/cfn-healthlake.yaml` | **DONE** |
| A12 | Generate .env file | `AWS-011-Generate-Env-File.yml` | N/A | **DONE** |
| A13 | Deploy-All wrapper | `AWS-099-Deploy-All.yml` | N/A | **DONE** |
| A14 | Destroy-All wrapper | `AWS-099-Destroy-All.yml` | N/A | **DONE** |

### Phase B: CloudFormation Templates — **ALL DONE**

All 10 templates created in `infra/` folder.

### Phase C: Agent Code — **ALL DONE**

5 platform-agnostic core agents + 5 AWS Lambda wrappers + shared models/signal_checker.
All in `DataManagement/SmartCareAgents/`.

### Phase D: Spring Boot AWS Integration — **ALL DONE**

| # | Task | Status |
|---|---|---|
| D1 | `application-aws-integration.yml` | **DONE** |
| D2 | `AwsConfigGenerator.java` | DEFERRED (manual .env download from GH Action artifact) |
| D3-D5 | `AwsSqsConsumer` (3 queues) | **DONE** (@Profile aws-integration) |
| D6 | `AwsStepFunctionsEngineFacade` | **DONE** (real AWS SDK StartExecution) |
| D7 | `npm run start:aws-integration` | **DONE** |

### Phase E: Admin UI Updates for AWS — **ALL DONE**

| # | Task | Status |
|---|---|---|
| E1 | AWS Step Functions engine seeded | **DONE** |
| E2-E3 | S3 dataset ingest | Already supported (AWS_S3 storage type exists) |
| E4 | AWS console link in detail page | **DONE** |
| E5 | Real-time results from SQS | **DONE** (AwsSqsConsumer saves to DB, UI reads from DB) |

### Phase F: Destroy Actions — **ALL DONE**

9 individual destroy workflows + 1 Destroy-All wrapper. All manual-trigger only.
